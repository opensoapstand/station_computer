...
DF_ERROR stateManualConfig::onAction()
{
   ...
   if (m_pMessaging->isCommandStringReadyToBeParsed())
   {
      ...
      {
         productDispensers[m_active_pump_index].the_pcb->SendByteToSlot(m_active_pump_index + 1, 0x01, 0b00100000); // Output pin values register (has no influence on input values)
         productDispensers[m_active_pump_index].the_pcb->SendByteToSlot(m_active_pump_index + 1, 0x03, 0b01011000); // Config register 0 = output, 1 = input (https://www.nxp.com/docs/en/data-sheet/PCA9534.pdf)
...
{
   uint8_t val;
   val = productDispensers[m_active_pump_index].the_pcb->readRegisterFromSlot(m_active_pump_index + 1, 0x03);
   debugOutput::sendMessage("Pin direction register 0x03: " + to_string(val) + "(" + uint8ToBinaryString(val) + ")", MSG_INFO);
   val = productDispensers[m_active_pump_index].the_pcb->readRegisterFromSlot(m_active_pump_index + 1, 0x01);
