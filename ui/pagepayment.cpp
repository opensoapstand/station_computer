...
//    idlePaymentTimer->start(60000);
    ...
    if (!payment){
        ...
        ui->order_drink_amount->setText("$" + QString::number(idlePage->currentProductOrder->getOrderPrice(), 'f', 2));
    } else {
    ...
    ui->order_total_amount->setText("$" + QString::number(idlePage->currentProductOrder->getOrderPrice(), 'f', 2));
...
void pagePayment::createOrder(){
    ...
    QString price = QString::number(idlePage->currentProductOrder->getOrderPrice(), 'f', 2);
...
//    ui->qrCode->setPixmap(map);
    ...
    //QString qrdata_amount = QString::number(idlePage->currentProductOrder->getOrderPrice(), 'f', 2);
...
//    qDebug() << "readingTimer_loop" << endl;
   ...
   // pktResponded = com.readPacket();
    ...
    usleep(100);
    com.flushSerial();
    pktToSend = paymentPacket.purchasePacket((QString::number(idlePage->currentProductOrder->getOrderPrice(), 'f', 2)).QString::toStdString());
    cout << "to PAY: " << ((QString::number(idlePage->currentProductOrder->getOrderPrice(), 'f', 2)).QString::toStdString());
