    ...
    ui(new Ui::DispenseWindow)
{
    ...
    ui->softButton->setMask(region); //masking the region to be circle for the button
    ui->softButton->setStyleSheet("border-image:url(:/assets/V5/circle_button.png); color: white;");
    QFont buttonFont("Arial", 20, QFont::Bold);
    ui->softButton->setFont(buttonFont);
...
void DispenseWindow::updateVolume()
{
    ...
    else if(progressValue >= 50 && paymentPage->getPriceSelect() == 1) //if the volume exceed
    {
        mainPage->sendardCommand("E");
        mainPage->sendardCommand("Z");
...
//        DispenseVolume.clear();
    ...
    else if(progressValue >= 360 && paymentPage->getPriceSelect() == 2) //if the volume exceed
    {
        mainPage->sendardCommand("E");
        mainPage->sendardCommand("Z");
        tempCount = 0; //reset temp counting
...
void DispenseWindow::on_softButton_pressed()
{
    mainPage->sendardCommand("P");
}

void DispenseWindow::on_softButton_released()
{
    mainPage->sendardCommand("S");
}
