...
void page_maintenance::showEvent(QShowEvent *event)
{
    ...
    for (uint8_t slot_index = 0; slot_index < MAINTENANCE_PAGE_SLOT_COUNT_MAX; slot_index++)
    {
        ...
        // QString product_status_text = p_page_idle->thisMachine->getSlotBaseProduct(slot_index+1)->getSlotStatusText();
        QString product_status_text = p_page_idle->thisMachine->getSlotByPosition(slot_index+1)->getSlotStatusText();
