/*******************************************************
 ...
 //
 // df_db_seed.sql
 // Test relations and future seeding
 //
 // created: 07-02-2020
 // by: Li Yan Tong
 //
...
update full_address = 'vancouver la';
...
insert into `product` (
		...
		`calibration_const`,
		`cost_per_litre`
	) value ('Elderflower Kombucha', NULL, 2.19, 1.20);
...
update `in_store_location` = 'main entrance';
...
insert into `pricing` (`product_id`, `price_per_litre`, `location`) value (1, 2.10, 1);
...
insert into `machine` (
		...
		`vendor_host`,
		`vendor_provider`
	) value (1, 1, 9, now(), 'City of vancouver', 'CCPD') on duplicate key
update `vendor_host` = 'vancouver la';
...
-- 			            value
--                         (1, 452.21);
insert into `inventory` (`product_id`, `machine_id`, `volume`) value (1, 1, 452.21);

-- inventory should always be logged individually so we can track progress
select *
from `inventory`;
